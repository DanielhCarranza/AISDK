...
final class AnthropicServiceSearchResultsTests: XCTestCase {
    ...
    func testTopLevelSearchResultsRequest() async {
        // Mock successful response with citations using requestHandler for reliable mocking
    ...
    func testToolBasedSearchResultsRequest() async {
        // Mock tool use response using requestHandler for reliable mocking
        MockURLProtocol.requestHandler = { request in
            ...
            return (response, mockToolUseResponse.data(using: .utf8)!)
        }
    ...
    func testCitationDecoding() {
        ...
        // Use the shared decoder that handles snake_case conversion
        let decoder = AnthropicHTTPClient.decoder
    ...
    func testSearchResultsWithoutBetaHeader() async {
        // Mock error response for missing beta header using requestHandler for reliable mocking
        MockURLProtocol.requestHandler = { request in
    ...
    func testCompleteSearchResultsWorkflow() async {
        ...
        MockURLProtocol.requestHandler = { request in
            ...
            return (response, mockFinalResponse.data(using: .utf8)!)
        }
